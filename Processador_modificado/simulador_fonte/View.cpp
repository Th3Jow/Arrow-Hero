...
{ unsigned int ir = model->getMem(pc),
  ...
  switch(model->pega_pedaco(ir,15,10))
	{ case INCHAR: 	sprintf(texto, "PC: %05d\t|	INCHAR R%d			|	R%d        <- teclado", 			pc, _rx, _rx);		 			 break;
		case OUTCHAR:	sprintf(texto, "PC: %05d\t|	OUTCHAR R%d, R%d	|	video[R%d] <- char[R%d]", pc, _rx, _ry, _ry, _rx); break;
		case CLS:	sprintf(texto, "PC: %05d\t|	CLS R%d, R%d	|	video[R%d] <- char[R%d]", pc, _rx, _ry, _ry, _rx); break;
	case MOV:
