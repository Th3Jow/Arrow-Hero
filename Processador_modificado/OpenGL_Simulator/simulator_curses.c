...
void show_program(WINDOW* codeWindow,int y, estado_da_maquina_curses estado) 
{
    ...
    switch(pega_pedaco(ir,15,10)){
        ...
        case OUTCHAR:
            mvwprintw(codeWindow,y,x, "PC: %05d  |  OUTCHAR R%d, R%d       |   VIDEO[R%d] <- CHAR[R%d] ", pc, rx, ry, ry, rx);
            break;

		case CLS:
            mvwprintw(codeWindow,y,x, "PC: %05d  |  CLS R%d, R%d       |   VIDEO[R%d] <- CHAR[R%d] ", pc, rx, ry, ry, rx);
            break;
